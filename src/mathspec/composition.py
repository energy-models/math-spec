...
A fragment is a :class:`~mathspec.model.Spec` of its own, and a base is one
too: each goes through :func:`~mathspec.validation.to_spec` before anything is
...
through :func:`~mathspec.validation.to_spec` like any other file.
...
def merge(fragments: Mapping[str, str | Path | Mapping[str, object] | Spec], description: str | None = None) -> Spec:
    ...
    Args:
        fragments: What each fragment is called, to the fragment: a YAML path,
            YAML text, a mapping, or a loaded :class:`~mathspec.model.Spec`.
...
def override(
    ...
    Args:
        base: The model being extended: a YAML path, YAML text, a mapping, or a
            loaded :class:`~mathspec.model.Spec`.
...
def _declarations(source: str | Path | Mapping[str, object] | Spec) -> dict[str, object]:
    ...
    Deliberately not :func:`~mathspec.validation.to_spec`: a patch carrying a
