...
def typeset_declaration(
    ...
    *,
    ...
    definition, a constraint, a ``piecewise:`` curve, or a variable's domain,
    quantifier included —
    ...
    Args:
        ...
        name: A named expression, constraint, ``piecewise:`` block or variable
            the model declares.
        fmt: What spells the math — a key of :data:`FORMATS`.
    ...
    if not found:
        ...
        msg = f"'{name}' is not a named expression, constraint, curve or variable. {did_you_mean(name, everything)}"
