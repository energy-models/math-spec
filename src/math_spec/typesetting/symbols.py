...
class Symbols:
    ...
    def __init__(self, schema: _ExpandedSpec, fmt: Format, table: SymbolTable) -> None:
        ...
        chosen = frozenset(schema.variables) | frozenset(schema.given.variables) | chosen_expressions(schema)
        names = (*schema.parameters, *schema.variables, *schema.given.variables, *schema.expressions)
        ...
        self.name: dict[str, str] = {
            ...
            else _derive_name_symbol(name, declared, fmt, upright=name not in chosen)
            for name in names
        }
        ...
        self.constraint: dict[str, str] = {
            name: table.names[name] if name in table.names else _derive_name_symbol(name, declared, fmt, upright=True)
            for name in (*schema.constraints, *schema.given.constraints)
        }
...
class SymbolTable:
    ...
    def checked_against(self, schema: _ExpandedSpec) -> SymbolTable:
        ...
        everything = (
            ...
            | set(schema.given.variables)
            | set(schema.expressions)
            | set(schema.constraints)
            | set(schema.given.constraints)
        )
        errors = [
