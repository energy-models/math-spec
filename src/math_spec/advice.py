...
def advice(model: str | Path | dict[str, Any] | Spec | Program) -> list[str]:
    """Every note decidable without data — never an error, one sentence each.
    ...
    Returns:
        The never-an-axis notes in declaration order, then the unboundedness
        notes.
    """
...
def _never_an_axis(program: Program) -> list[str]:
    """One note per dimension nothing is indexed by and nothing aggregates into."""
    axes: set[str] = set()
