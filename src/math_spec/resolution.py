...
class Namespace:
    ...
    __slots__ = ('constraints', 'dimensions', 'dtypes', 'leaf_dims', 'lookups', 'parameters', 'schema', 'variables')
...
class _Resolver:
    ...
    def _comparison(self, node: UnresolvedComparisonNode) -> WhereNode | UnresolvedWhereNode:
        """``name <op> literal``, or the two pair forms: ``lookup <op> lookup`` and ``parameter <op> parameter``.
        ...
        if node.name in ns.schema.expressions or (not node.quoted and value in ns.schema.expressions):
            right: ArithmeticNode = NameNode(value) if isinstance(value, str) else NumberNode(value)
            return self._expression_comparison(UnresolvedExpressionComparisonNode(NameNode(node.name), node.op, right))
        if not node.quoted and isinstance(value, str) and (rhs_kind := ns.kind(value)) is not None:
            ...
            if rhs_kind == 'parameter' and ns.kind(node.name) == 'parameter':
                if (refusal := _parameter_pair_error(context, node, value, ns)) is not None:
                    self.errors.append(refusal)
                    return node
                dims = (*ns.leaf_dims[node.name], *(d for d in ns.leaf_dims[value] if d not in ns.leaf_dims[node.name]))
