...
#: a walk's direction is written inside it, ``by=zone_of(generator -> zone)``,
#: and a sum with a ``by=`` takes no ``over=``. On ``shift`` and ``sum_back``
#: a ``by=`` partitions the axis the operator walks: it says which rows are
#: neighbours, not which group a term lands in, and ``by=cal(week)`` names the
#: columns whose values that group is read from.
