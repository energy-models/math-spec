...
| A `join` operator apart from `sum` and `at`, such as `sum(join(p, gen_bus), over=gen_bus[generator])` | a join opens an axis for each column it sums away, and the axis is not a dimension, so no declaration can carry it. One call joins and closes the axis, so no file can leave one open | `sum(p, over=generator, by=gen_bus[bus])` adds up each group, and `at(x, by=gen_bus[bus])` reads one row ([relations](../reference/language/relations.md#how-a-relation-is-used))  |
