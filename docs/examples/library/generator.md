...
`at(Port_p, by=Generator_port)` pins the flow at this component's own port
rather than adding a term to the balance, so the balance never grows.

The file is cut to what a dispatch model needs. A fixed build, no availability
profile and no ramp limits are three declarations PyPSA carries and this one
does not; [the PyPSA rungs](../pypsa.md) are where they are stated in full.
...
`Port_p` is one declaration again.
