...
> ✔ `pypsa 1.3.0` solves this rung's reference network at objective `12587.437500000098`, 60 rows. `lpspec 0.0.1.dev2+ge30be475d` binds `examples/pypsa_quadratic.yaml` against the same network and lands on the same objective (lpspec's parity gate). **Model-for-model**: the two lanes build one linopy model, label for label.
