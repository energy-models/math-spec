...
> ✔ `pypsa 1.3.0` solves this rung's reference network through its own linopy model at objective `12587.437500000098`, 60 rows — recorded by `examples/references/pypsa/rung_10_quadratic_costs.py`. `lpspec 0.0.1a259` binds `examples/pypsa_quadratic.yaml` against the same network and lands on the same objective, the same row and column count under every PyPSA name, and the same bus prices (`parity.py`). `lpspec.linopy` builds the very linopy model PyPSA builds, label for label (`structural.py`).
