...
> ✔ `pypsa 1.3.0` solves this rung's reference network at objective `7775.0`, 128 rows. `lpspec 0.0.1.dev5+g2efce5295` binds `examples/pypsa_linearized_uc.yaml` against the same network and lands on the same objective (lpspec's parity gate). Nodal prices agree on 8 rows.
