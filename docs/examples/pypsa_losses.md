...
> ✔ `pypsa 1.3.0` solves this rung's reference network at objective `11574.914894161273`, 150 rows. `lpspec gaf9a9b913` binds `examples/pypsa_losses.yaml` against the same network and lands on the same objective (lpspec's parity gate). Nodal prices agree on 20 rows.
