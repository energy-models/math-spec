...
key is every column. The declaration fixes no direction: the call says which
...
`sum` reads it with both ends named, and a bare `where` tests it. That is what
a many-to-many relation can say, and all it can say.

### Directions

A call reads a relation in one **direction**: it consumes one or more columns,
produces one or more, and joins on every other key column. The call writes both
ends. The operand carries every dimension consumed and every one joined on. The
result is what the operand had, less the consumed dimensions, plus the produced
ones.
...
- **A call names both of its ends, always.** `sum(p, by=gen_bus)` is refused,
...
- **A call brings the dimensions it lands on.** Producing one the operand
  already carries is refused. It would tie the operand's axis to the one it
...
- **One call reads one table.** `by=` names a single relation. To land on
  columns of two tables at once, declare one relation holding the columns of
  both; to read them in turn, write one call each.

Five refusals draw the line, and each message names the rewrite:

| refused                                | message                                                                                                                                                                                                                                                                  |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| a call with an end unsaid              | `sum() through a relation leaves into= unsaid. A call names both of its ends, so that a relation may gain a value column without changing what this call means. Write: sum(<expr>), sum(<expr>, over=<dim>) or sum(<expr>, by=<relation>, over=<column>, into=<column>)` |
| landing on a dimension already carried | `sum(by=gen_bus) lands on ['bus'], which the expression already carries. A call brings the dims it lands on, so that reading it tells you what it adds. Move the factor carrying ['bus'] outside the operator, or read to a column over another dimension.`              |
...
| a `sum` that consumes no key column    | `sum(by=zone_of): this sum lands on the key ['generator', 'period'], so each coordinate has one term and nothing is added up — that is a read, which is at()'s. Write at(..., by=zone_of, over=['generator'], into=['zone']), or sum toward a value column.`             |
| an operand missing a joined dimension  | `at(by=zone_of) joins on ['period'] (columns ['period'] of 'zone_of'), which the expression does not carry (dims ['zone']). A relation is read between two of its columns and joined at the others — index the operand by them, or read it between different columns.`   |

### Partitions

`shift(x, along=d, by=l)`, `sum_back(x, along=d, by=l)` and `position(d, by=l)`
step along the one key column over `d`, join on the other key columns, and group by
...
`snapshot`, so the frame is `[snapshot]` before the read and after it:
...
instead, which `sum` reads either way and nothing reads back. The rows where a
snapshot is its own
...
| relates members of two dimensions many-to-many, with nothing to weigh — which buses a generator may connect to                        | a bare `relation`, with no `values:`    | `sum` reads it with both ends named, and a bare `where` tests it. Nothing reads it back, because there is no one value to read |
