...
`where:` on the position keeps the first row out:
...
parameters:
  ...
  ramp: { dims: [] }
assumptions:
  load_ramps_within_limit:
    holds: "load - shift(load, over=snapshot, offset=1, edge=0) <= ramp"
...
compare one parameter against a literal or another parameter, or precompute
the test as a boolean parameter.
