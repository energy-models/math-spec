...
| lookup argument (`by=` on `sum` / `at`) | lookup — never a dimension; `from=` and `to=` name its columns                                                                 |
...
| `position(name, by=lookup[, from=col]) OP i` | a dimension and a lookup keyed over it | the same, counted **within each group** the lookup's value columns make — every period's first snapshot, whatever each period's length                                                                                                                                                                                                                                                                                                                                      |
