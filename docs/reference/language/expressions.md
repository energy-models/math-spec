...
| lookup argument (`by=` on `sum` / `at`) | a lookup, and never a dimension. `from=` and `into=` name its columns                                        |
...
| `name` (bare)                         | lookup                                 | A row exists: at the key for a keyed lookup, at every column for a bare relation. A `masked` lookup may be [partial](dimensions.md#coverage-says-whether-a-missing-key-was-meant), and this selects the keys that do map; on a `total` one every key has a row, so the bare name is a load error                                                                                                                         |
...
| `position(name, by=lookup[, into=c])` | a dimension and a lookup keyed over it | The same, counted within each group the lookup's value columns make                                                                                                                                                                                                                                 |
