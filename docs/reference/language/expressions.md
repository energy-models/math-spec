...
one dimension they draw from one label set. A label space is never one side of
such a comparison.
...
The lookup must be over the dimension being counted. Unlike `sum(by=)` and
`at(by=)`, it may be a label space, because counting lands no terms anywhere
([#280](https://github.com/energy-models/math-spec/issues/280)). A coordinate
the lookup sends nowhere is in no group. A group shorter than the position is an
error when the data binds, for the same reason as above.
