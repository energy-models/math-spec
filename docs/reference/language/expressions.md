...
| lookup argument (`by=` on `sum` / `at`) | lookup — never a dimension; `from=` and `into=` name its columns                                                               |
...
| `sum(x, by=l)`               | `(dims(x) − from(l)) ∪ into(l)`           | if `from(l) ⊄ dims(x)`, or a joined column's dim is not in `dims(x)`              |
| `sum(x, by=[l, m])`          | `(dims(x) − from(l)) ∪ into(l) ∪ into(m)` | the same, plus: if `l` and `m` consume different dims, or produce the same one    |
| `at(x, by=l)`                | `(dims(x) − from(l)) ∪ into(l)`           | if `from(l) ⊄ dims(x)`, a joined column's dim is not, or `l` has no key in `into` |
...
| `name` (bare)                              | lookup                                 | a row exists: at the key for a keyed lookup, at every column for a bare relation. A `masked` lookup may be [partial](dimensions.md#coverage-says-whether-a-missing-key-was-meant), and this is how a declaration asks for the keys that do map; on a `total` one every key has a row, and the bare name is a load error                                                                                                                                                     |
| `name OP value`, `name.col OP value`       | lookup                                 | a filter on a value column of a keyed lookup, read at its key — whose dimensions therefore have to be in the frame; the column is named where the key determines several. A null value is **false**, whatever the comparator                                                                                                                                                                                                                                                |
| `name OP name`, `name.a OP name.b`         | two lookup columns                     | the one comparison whose both sides are structure. Legal only where both keys are over the **same** dimensions _and_ both columns over one — `ends.bus0 != ends.bus1` excludes a self-loop                                                                                                                                                                                                                                                                                  |
...
| `position(name, by=lookup[, into=c]) OP i` | a dimension and a lookup keyed over it | the same, counted **within each group** the lookup's value columns make — every period's first snapshot, whatever each period's length                                                                                                                                                                                                                                                                                                                                      |
...
parameter in data prep — and neither is comparing two dimensions. Two _lookup
columns_ are the exception, and only two whose lookups are keyed over the same
dimensions and whose columns are over one: keyed alike, they are two columns
of one key table, so the comparison is a filter on that table rather than a
join between two, and over one dimension they draw from one label set, so a
match is possible at all. Keyed differently no row carries both, and over
different dimensions no value can ever match — either is a load error.
...
`by=` takes a lookup **with a key column over the dimension being counted** —
its value columns are the groups a row of that dimension is actually in. A row reads its own group's boundary, the
