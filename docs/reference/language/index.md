...
| 5   | Dimension sets compose by union. A constraint must _equal_ its `foreach`. An objective must carry no dimensions. A `where` or a bound must not _exceed_ its frame.                                                                                                                                                                                           | [How dimensions combine](expressions.md#how-dimensions-combine) |
...
| 8   | A missing value reads as whatever makes it contribute nothing: zero as a coefficient, and false in a `where`. Where no such reading exists, the language refuses it. A divisor and a bound are the two cases.                                                                                                                                                | [Absence](absence.md), [Operators](operators.md#shift)          |
...
| [Parameters, variables, constraints and the objective](declarations.md) | the four blocks that make up the math                                                                            |
| [Expressions](expressions.md)                                           | the two grammars, arithmetic and `where`: what a name may mean where, and how dimensions compose                 |
| [Reported expressions](reported.md)                                     | the entries that the math never reads. You read them off a solve, and the math-only restrictions no longer apply |
