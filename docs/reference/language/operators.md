...
This page lists the built-in operators, what each accepts, and what it
produces. The set is **closed**: these are all of them, and there is no
registry to add to, so a model cannot depend on what a caller registered.
...
| `sum_back(array, over=dim, within=n, by=lookup)`   | the window stays **inside each group** the lookup makes                                                                            |
| `dual(constraint)`                                 | the row dual of `constraint` over its own frame; only a [reported entry](reported.md#reading-a-constraints-dual) may call it       |

`array` is any expression of the right dim set, so each operator reads a
...
scalar. It is `sum(sum(x, over=a), over=b)` written once. An operand that is
already scalar is an error rather than a no-op, as is `over=` naming a dim the
...
lookup's values are the group labels, checked against the target dimension
when data binds. A group with no members contributes nothing, and a member
whose lookup value is null belongs to no group. An empty group holds a
**value** rather than a gap: on a comparison's constant side it reads zero,
where a coordinate the data never covered is refused ([absence](absence.md)).

## `at`

`at(x, by=l)` is the **adjoint of `sum(by=)`** and takes the same single
argument: the lookup names one mapping table, and the operator says which way
it is walked. `sum(by=)` consumes the dimension the lookup is over and produces
its target. `at` consumes the target and produces that dimension, reading one
coarse value once per fine label that points at it.

It reads a _variable_ as readily as a parameter: one decision taken per bus,
read once by every line that touches it. A fine label whose lookup value is
null reads nothing and its row is absent, matching `sum(by=)`'s null group.
...
delivery horizon. A width of `1` is `x` itself. The dimension **survives**,
unlike under `sum`: each position reads a window of its own.
...
`within=` takes a number or an **integer parameter**, never an expression. With
a parameter each entity gets a window of its own length. Two rules make a named
width mean one thing, and both are load errors:
...
own window, so no row is lost and nothing is vacated. A number there is a load
error, because adding a constant is something the expression can say for
itself. `edge='wrap'` makes the window reach around the axis instead, which is
what a representative period that repeats asks for.

## `shift`

`shift(x, over=d, offset=n)` is the value at _t−n_ along `d`, in the
dimension's **declared order**. `edge=` says what happens at the boundary.
...
`edge='wrap'` makes a battery cyclic without writing the boundary condition
out: the first snapshot reads the last.
...
- **Bare**: the vacated coordinate is **absent**. It propagates and the row it
  ...
  quantity starts at zero. An initial condition is then a row the model states,
...
- **Numeric**: the number stands where the slot was vacated, and the row
  survives. It is a number rather than a flag because the identity is
  positional, `0` for a sum and `1` for a product, and only the model can see
...
- **A bare `shift` over a variable-free expression is a load error.** A
  ...
  `x <= shift(dt, over=t, offset=1)` into `x <= 0`. The error names what it
  could have meant: `edge='wrap'`, `edge=0`, or `edge=0` **together with** a
  `where` excluding the vacated coordinate. Those last two are a pair, not a
  choice. A `where` alone does not lift the refusal, and `edge=0` alone leaves
...
Every `edge=` rule then reads the same, one group at a time. Bare, each
group's first coordinate is vacated and its row drops. `edge='wrap'` closes
**each group** onto its own last, which is what a store that returns to its
starting level every period asks for. `edge=v` puts `v` at each group's edge.
Without `by=`, `edge='wrap'` wraps the whole _axis_: the last coordinate of
the dimension feeds the first, which across periods means one period opening
on what another left.

`by=` takes a **groupable** lookup, one declaring `into:`, **over the
dimension being walked**. Its target is what a named `offset=` may vary over,
so each group is reached by its own; a label space targets nothing and is
refused ([#280](https://github.com/energy-models/math-spec/issues/280)). A
coordinate the lookup sends nowhere is in no group, so it reaches nothing and
no `edge=` speaks for it: the row drops under `edge=0` exactly as it does
bare.

`shift` reads parameters too, so `shift(dt, over=t, offset=1, edge=0)` is the
previous snapshot's duration without a pre-shifted copy of a table the model
already has.

### An offset that differs per entity

`offset=` may name an **integer parameter** instead of a number. Each entity
is then reached by its own offset: a construction lead time, a transit time, a
delay the source data carries as a column.
...
Three rules keep that a translation, each a load error naming its rewrite:
...
positions are absent exactly as a numeric one's are. A bare `shift` over a
**variable-free** operand stays refused, for the reason [above](#shift).
...
by that period's own lead time, each period's opening rows vacate by its own
...
A `(period, timestep)` grid says the same with the offset declared over
`period`, legal because `period` is not the axis being walked. Here it is legal
