...
`by=` takes a lookup that declares `into:`, over the dimension being walked. A
label space is refused
([#280](https://github.com/energy-models/math-spec/issues/280)), because the
target of the lookup is what a named `offset=` may vary over.
