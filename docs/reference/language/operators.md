...
| `sum(array, by=relation)`                            | The relation's key columns collapse onto its value columns                                                                                          |
| `sum(array, by=[relation, …])`                       | The same, onto every relation's value column. All the relations must consume the same dimension                                                       |
| `sum(array, over=relation.a, by=relation.b)`              | Key column `a` collapses onto value column `b`. The other key columns are joined on, so the array carries them and the result keeps them. Either side may be left unsaid, and is then the whole side |
| `sum(array, over=relation.[a, …], by=relation.[b, …])`    | The same with several columns on either side: consumed together, landed on a product                                                             |
| `at(array, by=relation)`                             | The relation's value columns are replaced by its key                                                                                          |
| `at(array, by=relation.a)`                     | Value column `a` is replaced by the key, one value per coordinate. A key column whose dimension `array` carries is read at the array's own coordinate. `a` may be a list |
...
| `shift(array, along=relation.k, offset=n[, within=c])` | The translation slides along key column `k` inside each group that the relation makes. Neighbours, edges and a wrap all belong to that group        |
...
| `sum_back(array, along=relation.k, window=n)`   | The window stays inside each group that the relation makes                                                                                          |
...
on the value column the key points at. A nodal balance is one `sum(by=)` per
kind of component, and the network's wiring stays in the relations:
...
`sum(by=)` consumes the key columns and produces the value columns. Where the
relation offers several, a column is written after its name: `over=l.k` for the
key columns consumed and `by=l.v` for the value columns landed on
([walks](dimensions.md#walks)). Every key column not consumed is joined on, so
each group is one coordinate of it. A bare relation, one with no `value:`, is
summed with the consumed columns named, `over=l.k`, and lands on the rest.
...
`at(x, by=l)` walks the same relation the other way. It consumes the value
columns and lands on the key, so it reads one coarse value once for each fine
label that points at it, and a bare relation is never read by `at`. `by=l.v`
names the value column read where two share a dimension. The key needs no
naming: a key column whose dimension `x` carries is read at the row's own
coordinate, and the rest are produced ([walks](dimensions.md#walks)).
...
`along=relation.k` keeps the window inside each group that the relation makes,
so no window reaches out of its own group. The relation obeys the rules given
for [`shift`](#a-translation-that-stops-at-each-groups-edge).
...
`along=relation.k` partitions the axis the operator slides along, so the
neighbour of a coordinate is the coordinate before it in its own group. A group can be a season, an
...
The column after the dot is a key column of the relation, and the axis is its
dimension. The group is the value columns: all of them, or the ones `within=`
names, so one calendar table serves `within=day` and `within=week` alike. The group columns are
...
Along a plain dimension, `edge='wrap'` wraps the whole axis: the last coordinate of the
...
[`along=relation`](#a-translation-that-stops-at-each-groups-edge) groups into. Then
...
| `sum(array, over=relation.a, by=relation.b)` | $`\sum_{g \in \mathcal{G} \,:\, \mathrm{zone\_of}(g,\ e) = z} p_{g,e} \ge \mathrm{demand}_{z,e} \qquad \forall\, z \in \mathcal{Z},\ e \in \mathcal{E}`$ |
| `sum(array, over=relation.[a, …], by=relation.[b, …])` | $`\sum_{g \in \mathcal{G},\ e \in \mathcal{E} \,:\, \mathrm{slot\_of.bus}(g,\ e) = b \wedge \mathrm{slot\_of.technology}(g,\ e) = t} p_{g,e} \le \mathrm{cap}_{b,t} \qquad \forall\, b \in \mathcal{B},\ t \in \mathcal{T}`$ |
...
| `shift(array, along=relation.k, offset=n)` | $`p_{t} \le p_{t \ominus^{\mathrm{season\_of}(t)} 1} \qquad \forall\, t \in \mathcal{T}`$ |
...
| `sum_back(array, along=relation.k, window=n)` | $`\sum_{h' \in \mathcal{H} \,:\, 0 \le h -^{\mathrm{day\_of}(h)} h' < 3} \mathit{started}_{u,h'} \le \mathit{on}_{u,h} \qquad \forall\, u \in \mathcal{U},\ h \in \mathcal{H}`$ |
...
$`t \ominus^{\mathrm{relation}(t)} k`$ denotes a translation counted inside the group a relation puts $`t`$ in (`shift(along=relation.d)`), so a term never crosses out of its own group.
<!-- operator-math:end -->
