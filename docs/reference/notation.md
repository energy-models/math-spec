...
Every construct the language has, beside the math the typesetter prints for
it, so the notation can be read as one system: two constructs that mean
different things look different, and a symbol is introduced where it is
defined. [Typesetting](typeset.md) says how to print a model.

`pixi run python -m tools.notation` generates the page, almost all of it from
one model:
...
That file is not a sensible optimisation problem; it is the one file that
carries every construct at once. Three checks in
`tests/typesetting/test_typeset.py` hold it to the language: every operator a
format spells, every node kind the parsers produce, every line of the walk. So
_every_ here is asserted, and a construct added to the language arrives on this
page or CI goes red. The curves are the exception, one real model per
`method:`.

This page is not the operator reference: what each operator _does_ is
[Operators](language/operators.md), which renders the same math one row per
call shape. It is not a tutorial: the models under `examples/` are written to
be read.

The symbols are the **derived** ones, taken with no symbol table, which is what
a model prints with no setup: $\mathit{load}_{t}$ rather than $\ell_t$. A
[symbol table](typeset.md#symbol-tables) replaces them wholesale and changes
