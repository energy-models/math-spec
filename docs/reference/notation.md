...
prints _all_ of it. It shows every construct the language has, beside the math
the typesetter gives that construct.

The point of putting them together is that you can read the notation as the one
system it has to be. Two constructs that mean different things have to look
different. A symbol has to be introduced where it is defined, and used where it
is meant.

This page is generated by `pixi run python -m tools.notation`. Almost all of it
...
That model is not a sensible optimisation problem, and it does not try to be. It
is the one file that carries every construct at once.

Three checks in `tests/typesetting/test_typeset.py` hold that file to the
language. They check every operator that a format spells, every node kind that
the parsers produce, and every line of the walk. So the word _every_ on this page
is asserted, not promised. A construct added to the language either arrives on
this page, or CI goes red.

The curves are the exception. They use one real model per `method:`, for the
reason that their section gives.

Two things this page is not. It is not the operator reference; what each operator
_does_ is [Operators](language/operators.md), which renders the same math with
one row per call shape. And it is not a tutorial; the models under `examples/`
are the ones written to be read.

The symbols here are the **derived** ones, taken with no symbol table, because
that is what a model prints with no setup. So you see $\mathit{load}_{t}$ rather
than $\ell_t$. A [symbol table](typeset.md#symbol-tables) replaces the symbols
wholesale, and changes nothing else on this page.
