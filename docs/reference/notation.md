...
balance:
  dims: [snapshot, bus]
  expression: sum(p, by=gen_bus(generator -> bus)) + spill - slack == load
```
...
seasonal_window:
  dims: [snapshot, generator]
  expression: sum_back(on, along=snapshot, window=3, by=season_of(season)) <= units
```
...
representative:
  dims: [snapshot]
  expression: sum(spill, by=rep_of(snapshot -> rep)) <= at(spill, by=rep_of(rep))
```
...
zonal_membership:
  dims: [snapshot]
  expression: sum(units, by=gen_zone(generator -> snapshot)) <= budget
```
...
zonal_pullback:
  dims: [snapshot, generator]
  where: "gen_zone == 'north' AND position(generator, by=gen_zone(zone)) == 0"
...
first:
  dims: [snapshot, generator]
  where: "position(snapshot) == 0 OR position(snapshot, by=season_of(season)) == 0"
  expression: on == 1
```
...
last:
  dims: [snapshot, generator]
  where: "position(snapshot) == -1 OR position(snapshot, by=season_of(season)) == -1"
  expression: on == 0
```
