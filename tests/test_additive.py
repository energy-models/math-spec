...
and `merge` sums the terms. The marked entry stays in the composed spec, so a
...
from mathspec import FORMATS, LanguageError, advice, merge, override, to_markdown, to_spec, typeset
...
def test_a_composed_model_takes_more_terms_in_a_second_merge():
    """The kept entry is what lets a framework ship a composed spec that a project adds to."""
...
def test_the_advice_says_the_files_that_add_terms_complete_the_sum():
    """A marked sum is completed by `merge`, not by a host model, so its note says so."""
    (note,) = [note for note in advice(BALANCE) if note.kind == 'given']
    assert note.subject == 'injection'
    assert 'a sum other files add terms to' in note.text
    assert 'layered onto' not in note.text, 'no host model is asked for'
