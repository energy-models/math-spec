...
"""One declaration typesets to the bare equation the whole-model render prints for it."""
...
from math_spec import LanguageError, SchemaError, typeset_equation
...
#: constraint reading the first — the shapes an equation has to read.
PLAIN = override(
    DISPATCH,
    **{
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            PLAIN,
            'budgeted',
            r'\mathit{spend}_{t} \le 10 \qquad \forall\, t \in \mathcal{T} \,:\, \mathrm{load}_{t} > 0',
            id='a-constraint-with-its-mask-on-the-quantifier',
        ),
        pytest.param(
            PLAIN,
            'p',
...
def test_a_declaration_prints_the_equation_the_whole_model_render_prints(model: dict, name: str, expected: str):
    ...
    assert typeset_equation(model, name, 'latex') == expected


@EVERY_FORMAT
def test_the_equation_is_bare_math_with_its_quantifier(name: FormatName, fmt: Format):
    """No label, no delimiters, no number: the caller lays it out. The
    quantifier stays, since without it a constraint's mask is lost."""
    equation = typeset_equation(PLAIN, 'budgeted', name)
    assert fmt.operators['forall'] in equation and fmt.operators['such_that'] in equation, 'quantifier and mask'
    assert 'budgeted' not in equation, 'the label is the name the caller already holds'
    assert not equation.startswith(('$', '\\begin')), 'math only, for a math context the caller controls'


def test_a_symbol_table_renames_an_expression_either_way():
    table = {'notation': 'latex', 'names': {'headroom': r'\bar h'}}
    assert typeset_equation(CASED, 'headroom', 'latex', symbols=table).startswith(r'\bar h_{t,g} =')
    table = {'notation': 'latex', 'names': {'spend': 'S'}}
    assert typeset_equation(PLAIN, 'spend', 'latex', symbols=table).startswith('S_{t} =')


def test_a_body_naming_another_expression_prints_its_symbol():
    """`spend` is named, so `double_spend` prints its symbol; the whole-model
    render defines both, each once, in declaration order."""
    model = override(PLAIN, **{'expressions.double_spend': 'spend * 2'})
    assert typeset_equation(model, 'double_spend', 'latex') == (
...
def test_a_name_declaring_no_equation_is_refused(name: str, match: str):
    with pytest.raises(SchemaError, match=match):
        typeset_equation(PLAIN, name, 'latex')
