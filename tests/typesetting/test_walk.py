...
def test_a_grouped_position_rides_a_subscript_rather_than_a_second_argument(name: FormatName, fmt: Format):
    ...
    """
    text = typeset(_selected('position(season_of.snapshot, within=season) == 0'), name)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            _selected('position(season_of.snapshot, within=season) == 0'),
...
def test_a_sum_that_reads_no_value_column_asks_only_that_the_key_has_a_row():
    """`sum(u, by=gen_zone.snapshot)` died with `KeyError: 'zone'`.
    ...
    """
    row = _grouped(['snapshot'], 'sum(u, by=gen_zone.snapshot) <= cap')
...
def test_a_value_column_the_call_consumes_is_a_condition_like_a_produced_one():
    """`sum(f, over=gen_bt.[generator, bus])` bound `b` and then
    ...
    key.
    """
    row = _grouped(['technology'], 'sum(f, over=gen_bt.[generator, bus]) <= cap')
    assert (
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            'p == at(sum(q, over=bus_of.generator), by=bus_of, over=bus, into=generator)',
