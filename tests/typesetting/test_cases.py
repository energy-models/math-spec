...
from math_spec import SchemaError, expand_piecewise, load_model, to_latex, typeset
...
#: One region and a fallback. `opening` is a column and the fallback a scalar,
#: so the arms alone would not give a quantity its shape — the `foreach` does.
BY_REGION = {
...
def test_a_case_reaching_a_variable_is_chosen(fmt: Format):
    """One arm holding a variable is enough: the solver decides the quantity."""
...
#: One cased expression reached only through another's arm. `opening_cost` has
...
def test_a_variable_reached_through_another_cased_expression_still_prints_chosen():
    ...
    """
    schema = expand_piecewise(load_model(_NESTED))
...
def test_the_definitions_print_in_declaration_order():
    ...
    schema = expand_piecewise(load_model(override(CASED, **{f'expressions.{n}': BY_REGION for n in declared})))
