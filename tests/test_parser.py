...
def test_a_parsed_node_pickles_and_stays_sealed():
    ...
    with pytest.raises(TypeError, match='does not support item assignment'):
        operator.setitem(copy.kwargs, 'consume', NameNode('generator'))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            lambda node: operator.setitem(node.left.kwargs, 'consume', NameNode('snapshot')),
            TypeError,
...
def test_a_parsed_expression_cannot_be_rewritten_under_another_pass(rewrite, error, match):
    ...
    `node.kwargs['consume'] = ...` re-aimed a reduction, with no error anywhere.
    """
...
def test_a_call_copies_the_kwargs_it_is_handed():
    ...
    passed = {'consume': NameNode('generator')}
    built = FunctionCallNode('sum', (NameNode('p'),), passed)
    passed['consume'] = NameNode('snapshot')
    assert built.kwargs == {'consume': NameNode('generator')}, 'the dict handed in was copied, not aliased'
