...
def test_a_program_carries_the_column_it_reads_apart_from_the_ones_it_builds():
    ...
    assert sorted(program.given_variables) == ['flow'], 'and binds each of these to a column it is given'
    assert program.given_variables['flow'].dims == ('snapshot', 'port'), 'the frame is what a binder checks'
...
LAYER = {
    ...
    'given_constraints': {'balance': {'dims': ['snapshot', 'bus'], 'description': 'the host clears each bus'}},
...
def test_the_program_carries_the_row_family_a_consumer_binds():
    ...
    assert sorted(program.given_constraints) == ['balance']
    assert program.given_constraints['balance'].dims == ('snapshot', 'bus'), 'the frame is what a binder checks'
...
def test_a_dual_naming_nothing_says_where_to_declare_it():
    ...
    with pytest.raises(LanguageError) as raised:
        to_spec(mistyped)
    message = str(raised.value)
    assert "'constraints:'" in message and "'given_constraints:'" in message, (
...
def test_a_given_row_family_is_refused_where_it_oversteps(block, says):
    with pytest.raises(LanguageError) as raised:
        to_spec({**LAYER, 'given_constraints': {'balance': block}})
    assert says in str(raised.value)
...
def test_merging_folds_a_row_family_into_the_file_that_builds_it():
    ...
    assert 'given_constraints' not in composed and 'given_variables' not in composed
    program = to_program(composed)
    assert sorted(program.constraints) == ['balance', 'cap']
    assert not program.given_constraints, 'nothing is left for a consumer to bind'
...
REACHED_ONLY_BY_A_GIVEN_FRAME = {
    ...
    'given_variables': {'flow': {'dims': ['port']}},
    'given_constraints': {'balance': {'dims': ['bus']}},
