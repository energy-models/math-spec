...
def test_every_kind_a_consumer_can_pin_against_is_produced_here():
    ...
    assert kinds == ADVICE_KINDS, 'every kind a consumer can pin against is one these two fixtures produce'


def test_the_three_kinds_come_in_one_order():
    all_three = override(
        READS_A_COLUMN,
        **{
            'dimensions.h': {'dtype': 'str'},
            'variables.slack': {'dims': ['g'], 'bounds': {'lower': -float('inf')}},
            'objective': {'sense': 'minimize', 'expression': 'sum(slack)'},
        },
    )
    assert [note.kind for note in advice(all_three)] == ['never-an-axis', 'given', 'unbounded'], (
        'the axis nobody reaches, then what a consumer binds, then what the objective pushes open'
    )
