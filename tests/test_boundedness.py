...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {'objective.expression': 'sum(shift(v, along=g, offset=1), consume=g)'},
            'lower',
...
THROUGH_EACH_OPERATOR = {
    ...
    'shift': {'objective.expression': 'sum(shift(v, along=g, offset=1), consume=g)'},
    'sum_back': {'objective.expression': 'sum(sum_back(v, along=g, window=2), consume=g)'},
    # `at` reads onto the relation's source, so the variable it drives is on `h`
    'at': {'variables.u': {'dims': ['h']}, 'objective.expression': 'sum(at(u, by=lk), consume=g)'},
}
