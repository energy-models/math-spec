...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "shift(p, along=snap_bus.snapshot, offset=bus_lead, edge='wrap')",
            {'snapshot', 'generator'},
            id='a-partition-makes-an-offset-over-another-dim-readable-one-lag-per-group',
        ),
        pytest.param(
            'sum_back(p, along=snap_bus.snapshot, window=bus_lead)',
            {'snapshot', 'generator'},
            id='a-partition-makes-a-width-over-another-dim-readable-one-window-per-group',
        ),
        pytest.param('p + 1', {'snapshot', 'generator'}, id='a-scalar-broadcasts'),
        pytest.param(
            'sum(load * p, by=gen_bus)',
            {'snapshot', 'bus'},
            id='a-landed-dim-the-operand-already-carries-is-joined-on-so-the-sum-is-a-masked-sum',
        ),
        pytest.param(
            'sum(p, over=gen_zone.generator)',
            {'snapshot', 'zone'},
            id='a-two-key-relation-sums-away-the-key-it-names-and-keeps-the-other',
        ),
        pytest.param(
            'sum(p, over=gen_zone.snapshot)',
            {'generator', 'zone'},
            id='the-same-table-summed-along-its-other-key',
        ),
        pytest.param(
            'zone_load[gen_zone]',
            {'snapshot', 'generator'},
            id='its-index-keeps-the-shared-key-too',
        ),
        pytest.param(
            "shift(p, along=gen_zone.generator, offset=1, edge='wrap')",
            {'snapshot', 'generator'},
            id='a-partition-along-one-key-joined-on-the-other',
        ),
        pytest.param(
            "shift(p, along=gen_bz.generator, offset=1, edge='wrap', within=bus)",
            {'snapshot', 'generator'},
            id='a-partition-grouped-by-one-value-column-of-a-two-value-table',
        ),
        pytest.param(
            'sum_back(p, along=gen_bz.generator, window=2, within=[bus, zone])',
            {'snapshot', 'generator'},
            id='a-window-grouped-by-both-value-columns-named',
        ),
        pytest.param(
            "shift(p, along=pair.g, offset=1, edge='wrap')",
            {'snapshot', 'generator'},
            id='a-partition-grouped-by-two-columns-over-one-dimension-lands-nothing',
        ),
        pytest.param(
            'sum(p, over=gen_bus.generator)', {'snapshot', 'bus'}, id='the-dot-is-legal-on-a-one-key-relation'
        ),
        pytest.param(
            'sum(p, by=gen_bz.[bus, zone])',
            {'snapshot', 'bus', 'zone'},
        ...
        pytest.param(
            'sum(p, over=gen_zone.[generator, snapshot])',
            {'zone'},
            id='a-key-list-sums-two-key-columns-away-at-once',
        ),
        pytest.param(
            'sum(p, by=gen_bz.bus)',
            {'snapshot', 'bus'},
        ...
        pytest.param(
            'sum(p, by=rep_of.rep)', {'snapshot', 'generator'}, id='a-map-into-its-own-dimension-keeps-the-frame'
        ),
        pytest.param('p[rep_of]', {'snapshot', 'generator'}, id='and-so-does-its-index'),
        pytest.param(
            "shift(p, along=rep_of.snapshot, offset=1, edge='wrap')",
            {'snapshot', 'generator'},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            'sum(load, by=gen_bus)',
            r"sum\(by=gen_bus\) reads \['generator'\], the dims it consumes",
        ...
        pytest.param(
            'sum(cost, over=gen_zone.generator)',
            r"sum\(over=gen_zone\) joins on \['snapshot'\]",
            id='a-grouped-sum-needs-the-keys-it-joins-on',
        ),
        pytest.param(
            'cost[gen_zone]',
            r'reads \[.zone.\], the dims it consumes',
            id='an-index-needs-the-value-dim-it-reads',
        ),
        pytest.param(
            "shift(cost, along=gen_zone.generator, offset=1, edge='wrap')",
            r"along=gen_zone\) joins on \['snapshot'\]",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param('position(rep_of.snapshot) == 0', {'snapshot'}, id='a-position-within-a-representative'),
        pytest.param(
            'position(gen_zone.generator) == 0',
            {'generator', 'snapshot'},
            id='a-position-within-a-group-of-a-two-key-relation-reads-both-keys',
        ),
        pytest.param(
            'position(gen_bz.generator, within=zone) == 0',
            {'generator'},
