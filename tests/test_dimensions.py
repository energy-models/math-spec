...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            'sum(p, by=gen_zone.generator)',
            {'snapshot', 'zone'},
            id='a-two-key-lookup-consumes-the-key-it-walks-and-keeps-the-other',
        ),
        pytest.param(
            'sum(p, by=gen_zone.snapshot)',
            {'generator', 'zone'},
            id='the-same-table-walked-along-its-other-key',
        ),
        pytest.param(
            'at(zone_load, by=gen_zone.generator)',
            {'snapshot', 'generator'},
            id='its-pullback-keeps-the-joined-key-too',
        ),
        pytest.param(
            "shift(p, over=generator, offset=1, edge='wrap', by=gen_zone.generator)",
            {'snapshot', 'generator'},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            'sum(cost, by=gen_zone.generator)',
            r"sum\(by=gen_zone.generator\) joins on \['snapshot'\], which the expression does not carry",
            id='a-grouped-sum-needs-the-keys-it-joins-on',
        ),
        pytest.param(
            'at(zone_cap, by=gen_zone.generator)',
            r"at\(by=gen_zone.generator\) joins on \['snapshot'\]",
            id='a-pullback-needs-the-keys-it-joins-on',
        ),
        pytest.param(
            "shift(cost, over=generator, offset=1, edge='wrap', by=gen_zone.generator)",
            r"by=gen_zone.generator\) joins on \['snapshot'\]",
            id='a-partition-needs-the-keys-it-joins-on',
        ),
        pytest.param(
            "shift(p, over=generator, offset=1, edge='wrap', by=gen_zone.snapshot)",
