...
def test_the_form_declares_the_same_model(path):
    ...
    """
    original, rewritten = (ms.to_program(source) for source in (path, ms.to_spec(path).to_yaml(canonical=True)))
    for group in ('parameters', 'variables', 'constraints', 'named_expressions', 'piecewise', 'sos', 'dimensions'):
